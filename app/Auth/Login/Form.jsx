  ...
  return (
    <div className='w-full h-full p-6  bg-black/40 z-[10] '>
        ...
        <div className="w-full h-full flex  justify-center gap-10 flex-col items-center">
            <div className="flex flex-col  px-7 items-center  rounded-xl bg-black/60 backdrop-blur-[10px] py-8 h-[690px] w-[525px]">
                ...
                <div className="flex w-full max-w-[90%] justify-center flex-col gap-10 pt-20">
                    ...
                    <div className="flex flex-col gap-[16px] w-full">
                        <div className="flex flex-col gap-3  w-full ">
                            ...
                            <input placeholder="you@example.com" value={formData.email} name='email'
                            onChange={handleChange} type="text" className='flex px-4 outline-none hover:border-white duration-200 ease-in-out placeholder:text-white/50 text-white gap-3 h-[52px] text-[15.5px] w-full items-center justify-center border-white/70 border-[.5px] rounded-full' />
                        </div>
                        <div className="flex flex-col gap-3  w-full ">
                            ...
                            <div className="grid grid-cols-[1fr_auto]  px-4 outline-none hover:border-white duration-200 ease-in-out placeholder:text-white/50 text-white gap-[10px] h-[52px] text-[15.5px] w-full items-center justify-center border-white/70 border-[.5px] rounded-full">
                                ...
                                onChange={handleChange}  type={showPassword ? "text" : "password"} className={`outline-none ${!showPassword ? 'text-[21px] font-Montserrat' : ''}  placeholder:text-[21px] border-none w-full`} />
                                {
                                    formData.password && (
                                        ...
                                        <button className='cursor-pointer text-[17px]' onClick={() => setShowPassword((prev) => !prev)}>{showPassword ? <BsEye /> : <BsEyeSlash />}</button>
                                    )
                                }
                            </div>
                        </div>
                        <button  type="submit" disabled={!isEmailAndPasswordValid}  className={`flex ${!isEmailAndPasswordValid ? 'bg-main/15 text-white/30 border-main/15 cursor-not-allowed' :  'bg-white text-main2 border-white/50 cursor-pointer'} hover:bg-green   mt-1 text-main2 font-bold gap-3  duration-200 ease-in-out h-[52px] w-full items-center justify-center  border-[.5px] rounded-full`}>Enter the Stüdyo</button>
