...
export default function LatestAlbums() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/albums/latest', {
      headers: {
  ...
  return (
    <section className="relative -top-3">
      <div className="flex pb-3 flex-col gap-2">
        <h1 className='text-2xl flex gap-[7px] items-start  hover:underline ease-in-out duration-200 w-fit cursor-pointer   text-white/95 font-NeueMontreal font-semibold'><span className='bg-[#c42b1c] relative top-1.5 w-[2px] h-[20px] '></span>Latest Albums</h1>
        </div>
        ...
        <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className="w-full h-auto relative">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {albums.map(album => (
                      <Link href={`/album/${album.public_id}`} key={album.id} className="group">
                        ...
                        <h3 className="mt-2 text-white font-semibold text-sm line-clamp-1">{album.title}</h3>
                        <p className="text-white/70 text-xs line-clamp-1">{album.user.username}</p>
                      </Link>
                    ))}
                  </div>
