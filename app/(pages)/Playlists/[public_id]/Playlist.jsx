...
const Playlist = () => {
  ...
  useEffect(() => {
    if (public_id) {
      axios.get(`http://localhost:8000/api/playlists/${public_id}`, {
   ...
   const handlePlayAll = () => {
    const formattedQueue = playlist.tracks.map(track => ({
     id: track.id,
      title: track.title,
        ...
        <div className="flex gap-5 items-end">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-1 items-center">
                <img className='size-[27px]  rounded-full  object-cover ' src={`http://localhost:8000/${playlist.user.profile_image}`} alt="" />
                <Link href={`/artist/${playlist.user.public_id}`} className='text-white/95 hover:underline  font-semibold capitalize tracking-wide font-NeueMontreal text-[14px]'>{playlist.user.username} </Link>
                <span className='size-1 relative top-[2px]  rounded-full bg-white/80'></span>
                {/* <div className="text-white/80  font-medium  tracking-wide font-NeueMontreal text-[13.5px]">{new Date(playlist.release_date).getFullYear()}</div>
                <span className='size-1 relative top-[2px]  rounded-full bg-white/80'></span> */}
                <h4 className='text-white/80 flex items-end gap-1  font-medium  tracking-wide font-NeueMontreal text-[13.5px]'>  {playlist.tracks.length} songs </h4>
            </div>
          </div>
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <div className="size-[50px] flex-center rounded overflow-hidden relative">
                            ...
                            <div className="flex flex-col leading-snug">
                                ...
                                <Link href={`/artist/${playlist.user.public_id}`} className="flex hover:underline hover:text-white items-center gap-4">
                                    <span className="font-normal text-[#fff]/80 font-NeueMontreal text-[14px]">{playlist.user.username}</span>
                                </Link>
                            </div>
                        </div>
