...
export default function Gallery() {
  ...
  // Transform mouse position into image movement
  const leftX = useTransform(mouseX, [0, window.innerWidth], [-20, 20])
  const leftY = useTransform(mouseY, [0, window.innerHeight], [-15, 15])
  const rightX = useTransform(mouseX, [0, window.innerWidth], [20, -20])
  const rightY = useTransform(mouseY, [0, window.innerHeight], [-15, 15])
  ...
  return (
    <div 
      ref={containerRef}
      className="w-full relative grid grid-rows-[1fr_auto] text-[#03050a] py-[60px] h-screen overflow-hidden"
    >
      <div className="flex items-center pt-[85px] gap-2 flex-col">
        <h1 className='cursor-pointer relative px-6 w-fit font-NeueMontreal uppercase leading-[.85] text-[#d8262c] font-[600] tracking-[0.015em] text-[7vw]'>
          New Discoveries 
          <h1 className='cursor-pointer absolute textx top-0 left-0 z-[100] px-6 w-fit font-NeueMontreal uppercase leading-[.85] text-[#d8262c] font-[600] tracking-[0.015em] text-[7vw]'>
            New Discoveries 
          </h1>
        </h1>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="w-[260px] top-[48%] -translate-y-1/2 left-[28%] absolute h-[380px] z-40"
        >
          <motion.img
            key={active.left}
            src={active.left}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="w-[260px] top-[48%] -translate-y-1/2 right-[29%] absolute h-[380px] z-40"
        >
          <motion.img
            key={active.right}
            src={active.right}
